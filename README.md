# JetLend Tasks
[Демо на GitHub Pages](https://helenvirtanen.github.io/JetLend/)

## Задача 1: разработка компонента

**Каталог пользователей**
![UsersCatalog](./public/screenshots/JetLendTask1.PNG)

### 📖 Описание
1) Функция получает данные из всех JSON-источников и предоставляет единый массив пользователей с суммарной оценкой по задачам и разбивкой по проектам.  
2) В каждом проекте пользователя собран список его задач.  

3) Поля результата
- `name` — имя пользователя  
- `total_estimate` — суммарная оценка по всем задачам  
- `projects_estimates` — массив проектов пользователя:
  - `name` — название проекта  
  - `total_estimate` — суммарная оценка задач проекта  
  - `tasks` — массив задач проекта  

### Источники данных
- [users.json](https://jetlend.ru/media/files/hr/frontend/users.json)  
- [projects.json](https://jetlend.ru/media/files/hr/frontend/projects.json)  
- [tasks.json](https://jetlend.ru/media/files/hr/frontend/tasks.json)  

### Пример результата
```json
[
  {
    "name": "Мария Иванова",
    "total_estimate": 35,
    "projects_estimates": [
      {
        "name": "Alpha Development",
        "total_estimate": 25,
        "tasks": [
          { "id": 1, "userId": 1, "projectId": 1, "estimate": 10 },
          { "id": 2, "userId": 1, "projectId": 1, "estimate": 15 }
        ]
      }
    ]
  }
]
```

## Задача 2: Модальное окно с подтверждением

**Открытое модальное окно**  
![Modal Opened](./public/screenshots/JetLendTask2.PNG)

**После подтверждения**  
![Modal Closed](./public/screenshots/JetLendTask3.PNG)


### 📖 Описание
При нажатии кнопки **«Выполнить действие»** открывается модальное окно с подтверждением:

- **Заголовок:** «Согласие с правилами»  
- **Крестик** для закрытия в правом верхнем углу  
- **Текст:**  
  *«Для данной функции применяются особые условия и правила пользования, их необходимо подтвердить, нажав на кнопку Подтвердить»*  
- **Кнопки:** «Отмена» и «Подтвердить»  
  - Кнопка **«Подтвердить»** заблокирована первые 5 секунд  
  - Текст кнопки показывает таймер: `Подтвердить (5)` … `Подтвердить`  
- После подтверждения окно закрывается и появляется сообщение *«Действие выполнено»*  
- Повторное нажатие кнопки **«Выполнить действие»** сразу выполняет действие, если пользователь уже подтвердил


## 🛠️ Стек
- React
- TypeScript
- Vite
- Vitest + Testing Library

## 🚀 Установка и запуск
**Локально**
1. Клонируйте репозиторий:
git clone https://github.com/HelenVirtanen/JetLend
2. Установите зависимости:
``` 
npm i
```

3. Запустите приложение:
```
npm run dev
```
   
Приложение будет доступно по адресу http://localhost:5173

4. Запустить тесты: 
```
npm run test
```

**Отчет по тестам**
![Test report](./public/screenshots/JetLendTask4.PNG)